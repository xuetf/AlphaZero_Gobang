...
def run(config=None):
    ...
    try:
        ...
        tool = VisualTool(board_size=[config.board.width, config.board.height])
        ...
        tool.set_player(player1, player2) if who_first == 0 else tool.set_player(player2, player1)


        # --------------------3.start game--------------------#
        game = Game(board, tool=tool)
